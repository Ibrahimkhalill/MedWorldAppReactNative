...
function CoursesDocument({ navigation }) {
  ...
  const [courseData, setCousesData] = useState([]);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.status === 200) {
        ...
        fetchCourseData(); // Refresh the course list after deletion
      }
    } catch (error) {
      console.error("Error deleting course:", error);
      Alert.alert("Error", "Could not delete course. Please try again later.");
    }
  };
  const confirmDelete = (item) => {
    Alert.alert(
      "Confirm Deletion",
      `Are you sure you want to delete the course "${item.name}"?`,
      [
  ...
  const generatePdf = async () => {
    try {
      ...
      const downloadUri = `${FileSystem.documentDirectory}CoursesReport.pdf`;
